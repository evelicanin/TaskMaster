...
<?php $task_mailovi_set = find_task_mail_list_admin($id_task); ?>

<?php
if (isset($_POST['submit']))
{
	$oper = (int)$_POST['operater'];
	$stat = (int)$_POST['status'];

	$rok_taska = $_POST['date'];
    $rok=date('Y-m-d',strtotime($rok_taska));

	//$_SESSION["message"] = $oper.' '.$stat  ;//PROVJERA

		// pozivamo funkciju za update taska od strane admina
		$update_result = admin_update_task($oper,$stat,$rok,$id_task);
		...
		if ($update_result && mysqli_affected_rows($connection) >= 0)
		{
		  ...
		  $akcija = 'IZMJENA TASKA | status : '.$task_after_update["status"].' | operater : '.$task_after_update["ime_operatera"].' | rok taska : '.date('d-m-Y', strtotime($task_after_update["task_rok"]));
      ...
      // slanje maila prema korisniku, operateru i adminu da je nesto izmjenjeno
			// ovo su aderse na koje treba da ide mail o izmjeni taska
				...
				$subject = "IZMJENA TASKA OD STRANE ADMINISTRATORA";
		   	...
		   	$message  = "----------------------------------------------------------------------------------------------------------------------";
				...
				$message .= '<br />';
				$message .= 'rok taska : '.date('d-m-Y', strtotime($task_after_update["task_rok"]));
				$message .= '<br /><br />';
       ...
       header("Refresh: 1.5; URL=$url1");
		    ...
		    $_SESSION["message_level"] = "green";
		  	...
		  	//$_SESSION["message"] .= "Poslani su mailovi na adrese : ".$to;
	...
	<div class="container">
		...
		<div id="myTabContent" class="tab-content">
				...
				<div class="panel panel-success">
				  ...
				  <div class="panel-body">
						...
						<form id="novi_task" action="" method="post" name="task_update">
							<ul>
								<li>
									<label for="operater">Dodijeljeni operater taska</label>
									<select name="operater">
										<option>
											<?php
												  echo $task["id_operatera"];
												  echo ' ';
												  echo $task["ime_operatera"];
											?>
										</option>
										<?php
								...
								<li>
									<label for="status">Predviđeni rok završetka</label>
									<input type="text" value =
									"
									<?php
									    if (isset($task["task_rok"]))
									    {
											   //echo $task["task_rok"];
											   echo(date('d-m-Y', strtotime($task["task_rok"])));
											}
									...
									?>
									"
									name="date" id="date" data-select="datepicker">
								</li>
            ...
            echo '<strong style="text-transform:uppercase;">';
				...
				<form class="form-horizontal" id="dodaj_komentar" action="dodaj_komentar_kao_admin.php" method="get" name="dodaj_komentar">
				  <fieldset>
